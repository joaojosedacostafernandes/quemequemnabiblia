...
Projeto iniciado em 2026-08-16, já com 6 rondas de trabalho integradas em `master`. Última atualização: 2026-08-18, fim da Ronda 6 (Ezequiel e os 12 Profetas Menores). Contexto para continuar este projeto noutra sessão.
...
- `data/personagens.json` — os dados: 97 personagens (`personagens[]`, cada um com id/nome/tipo/retrato/tier/x/y/era/refs/resumo/contexto/relacoes) e 77 ligações (`edges[]` — pai/mãe, casamento, irmãos, "várias gerações depois"). `layout` é `{ width: 3800, height: 820 }` — tem de bater certo com o `viewBox` do `<svg id="graph">` em `index.html`.
