...
Projeto iniciado em 2026-08-16, já com 6 rondas de conteúdo integradas em `master` mais uma Ronda 7 de redesign de navegação (Sub-projeto A) numa branch própria. Última atualização: 2026-08-19, fim da Ronda 7 / Sub-projeto A (redesign de navegação: percurso → cluster). Contexto para continuar este projeto noutra sessão.
...
- `data/personagens.json` — os dados: 97 personagens (`personagens[]`, cada um com id/nome/tipo/retrato/tier/x/y/era/refs/resumo/contexto/relacoes) e 77 ligações (`edges[]` — pai/mãe, casamento, irmãos, "várias gerações depois"). **Desde a Ronda 7, os campos `x`/`y` de cada personagem e o bloco `layout` de topo (`{ width, height }`) são vestigiais** — continuam no ficheiro mas já não são lidos por nada: a posição de cada personagem é agora calculada em tempo real a partir da era, das ligações familiares e do `tier`. O ficheiro tem também um novo array de topo `eras` (33 entradas, `{nome, descricao}`) que define a ordem narrativa usada pelo ecrã do percurso.
...
Cada ronda de conteúdo (Rondas 2 a 6) seguiu o mesmo fluxo, que resultou bem e vale a pena repetir:
...
3. Expansão de conteúdo: Génesis, Êxodo, Juízes, Rute, Samuel, Reis (reino dividido), Isaías, Jeremias, Ezequiel e os 12 profetas menores já estão cobertos — 97 personagens no total. **Deliberadamente fora da Ronda 6** (para uma ronda futura de exílio/pós-exílio): Zorobabel e o sumo sacerdote Josué (Jesua), ligados a Ageu/Zacarias mas mais naturalmente tratados junto de Esdras/Neemias/Ester/Daniel, onde a reconstrução do Templo é aprofundada. Falta o resto da Bíblia: exílio/pós-exílio, Evangelhos, Atos, Cartas.
...
Memórias persistentes em `~/.claude/projects/.../memory/`, atualizadas até ao fim da Ronda 6:
